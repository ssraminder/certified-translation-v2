...
export default function Page({ initialAdmin }){
  ...
  useEffect(() => {
    ...
    // Real-time polling for price updates (every 2 seconds)
  ...
  // Memoized computed totals for real-time updates
    ...
    <AdminLayout title="Quote Details" initialAdmin={initialAdmin}>
          ...
          <div className="flex-1 lg:w-[65%] space-y-8">
            ...
            <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                ...
                {canEdit && (
                  <div className="flex items-center gap-2">
                    <button onClick={()=> setShowAddItem(true)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm font-medium hover:bg-gray-50">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 16 16">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.33} d="M4.185 8h9.333M8.852 3.333v9.334"/>
                      </svg>
                      Add Item
                    </button>
                    <button onClick={()=> setShowDiscount(true)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm font-medium hover:bg-gray-50">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 16 16">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.33} d="M3.474 8h9.333M8.14 3.333v9.334"/>
                      </svg>
                      Discount
                    </button>
                    <button onClick={()=> setShowSurcharge(true)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm font-medium hover:bg-gray-50">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 16 16">
              ...
              <div className="space-y-4">
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-gray-600">Applied to: ${Number(computedTotals.subtotal||0).toFixed(2)} subtotal = -${Number(a.total_amount||0).toFixed(2)}</p>
                                {a.notes && <p className="text-sm text-gray-500 mt-1">{a.notes}</p>}
                              </div>
                            </div>
                            {canEdit && (
                ...
                {additionalItems.length === 0 && discounts.length === 0 && surcharges.length === 0 && (
      ...
      <DiscountModal
        ...
        onSubmit={async ({ description, discount_type, discount_value }) => {
          ...
          await addAdjustment({ type:'discount', description, discount_type, discount_value, subtotal: Number(computedTotals.subtotal||0), is_taxable:false });
        }}
      />
      <SurchargeModal
        open={showSurcharge}
        ...
        onSubmit={async ({ description, discount_type, discount_value }) => {
          ...
          await addAdjustment({ type:'surcharge', description, discount_type, discount_value, subtotal: Number(computedTotals.subtotal||0), is_taxable:true });
        }}
      />
      <EditLineItemModal
        open={showEditLine}
