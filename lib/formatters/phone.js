...
function normalizePlusPrefixed(input){
  ...
  if (digits.length < 6 || digits.length > 15) return null; // E.164 6-15 digits after +
  return `+${digits}`;
}
...
export function toE164(input, country){
  ...
  if (c === '1') {
    // US/CA exactly 10 national digits
    if (digits.length === 10) return `+1${digits}`;
    if (digits.length === 11 && digits.startsWith('1')) return `+${digits}`;
    return null;
  }
  if (c) {
    // Other countries: 6-15 national digits
    if (digits.length >= 6 && digits.length <= 15) return `+${c}${digits}`;
    // If pasted with country code but without + (total length up to 15)
    if (digits.length >= 7 && digits.length <= 15) return `+${digits}`;
    return null;
  }
  // Fallback when country unknown
  if (digits.length === 10) return `+1${digits}`;
...
export function parseE164(e164){
  const m = String(e164||'').match(/^\+(\d{1,3})(\d{1,14})$/);
  if (!m) return null;
  return { callingCode: `+${m[1]}`, national: m[2] };
}
