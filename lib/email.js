...
import { generateQuoteReadyHTML, generateQuoteReadyText } from './emailTemplates/quoteReadyEmail';
...
export async function sendAdminWelcomeEmail({ email, full_name, role }) {
  ...
  <div style="max-width:640px;margin:0 auto;padding:24px;">
    <div style="background:#fff;border-radius:10px;padding:28px;border:1px solid #e6ecf1;">
      <h1 style="margin:0 0 8px;color:#111;">Welcome, ${simpleEscape(firstName)}!</h1>
      <p>Your admin account has been created.</p>
      <ul>
        <li><strong>Role:</strong> ${simpleEscape(role || '')}</li>
        <li><strong>Email:</strong> ${simpleEscape(email)}</li>
      </ul>
      <p>To log in, visit <a href="${SITE_URL}/login">${SITE_URL}/login</a> and enter your email to receive a login code or magic link.</p>
      <p style="color:#6b7280;font-size:13px;">© ${new Date().getFullYear()} ${simpleEscape(FROM_NAME)}</p>
    </div>
  </div>
</body></html>`;
  try {
    ...
    sendSmtpEmail.sender = { email: FROM_EMAIL, name: FROM_NAME };
    sendSmtpEmail.htmlContent = html;
    sendSmtpEmail.tags = ['admin-welcome'];
    const resp = await apiInstance.sendTransacEmail(sendSmtpEmail);
    const messageId = resp?.messageId || resp?.response?.body?.messageId || null;
    await logEmailSent({ email_type: 'admin_welcome', recipient: email, provider_id: messageId, status: 'sent' });
    return { success: true, messageId };
  } catch (error) {
    await logEmailSent({ email_type: 'admin_welcome', recipient: email, status: 'failed', error_message: error.message });
    return { success: false, error };
  }
}
...
export async function sendQuoteReadyEmail({ email, first_name, quote_number, quoteData, viewQuoteUrl, settings }) {
  if (!email) return { success: false, error: new Error('Missing recipient email') };

  try {
    const subject = `Your Translation Quote is Ready - Quote #${simpleEscape(quote_number || '')}`;

    const htmlContent = generateQuoteReadyHTML({
      logoUrl: settings?.logo_url?.value || null,
      companyName: settings?.company_name?.value || FROM_NAME,
      companyEmail: settings?.support_email?.value || FROM_EMAIL,
      companyPhone: settings?.support_phone?.value || '+1 (555) 123-4567',
      businessHours: quoteData?.businessHours || [],
      firstName: first_name || 'there',
      quoteNumber: quote_number || '',
      estimatedDelivery: quoteData?.estimated_delivery_date || settings?.default_turnaround_time?.value || '3-5 business days',
      quoteExpires: quoteData?.quote_expires_at ? new Date(quoteData.quote_expires_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '',
      languages: quoteData?.languages || '',
      documentCount: quoteData?.document_count || 0,
      pageCount: quoteData?.page_count || 0,
      certifications: quoteData?.certification_type || '',
      lineItems: quoteData?.lineItems || [],
      subtotal: quoteData?.subtotal || 0,
      tax: quoteData?.tax || 0,
      total: quoteData?.total || 0,
      viewQuoteUrl,
      upcomingHoliday: quoteData?.upcomingHoliday || null
    });

    const textContent = generateQuoteReadyText({
      firstName: first_name || 'there',
      quoteNumber: quote_number || '',
      estimatedDelivery: quoteData?.estimated_delivery_date || settings?.default_turnaround_time?.value || '3-5 business days',
      quoteExpires: quoteData?.quote_expires_at ? new Date(quoteData.quote_expires_at).toLocaleDateString() : '',
      languages: quoteData?.languages || '',
      documentCount: quoteData?.document_count || 0,
      pageCount: quoteData?.page_count || 0,
      certifications: quoteData?.certification_type || '',
    ...
    sendSmtpEmail.htmlContent = htmlContent;
    sendSmtpEmail.textContent = textContent;
    sendSmtpEmail.tags = ['quote-ready', 'transactional'];
...
export async function sendHitlQuoteReadyEmail({ email, first_name, quote_number, pricing }) {
  if (!email) return { success: false, error: new Error('Missing recipient email') };
  const subject = `Your Translation Quote is Ready - Quote #${simpleEscape(quote_number||'')}`;
  const rows = [
    `<div style="display:flex;justify-content:space-between;padding:6px 0;"><span>Translation</span><span>$${Number(pricing?.translation||0).toFixed(2)}</span></div>`,
    `<div style="display:flex;justify-content:space-between;padding:6px 0;"><span>Certification</span><span>$${Number(pricing?.certification||0).toFixed(2)}</span></div>`,
    `<hr style="border:none;border-top:1px solid #e5e7eb;margin:8px 0;" />`,
    `<div style="display:flex;justify-content:space-between;padding:6px 0;"><span>Subtotal</span><span>$${Number(pricing?.subtotal||0).toFixed(2)}</span></div>`,
    `<div style="display:flex;justify-content:space-between;padding:6px 0;"><span>Tax</span><span>$${Number(pricing?.tax||0).toFixed(2)}</span></div>`,
    `<div style="display:flex;justify-content:space-between;padding:8px 0;font-weight:700;"><span>Total</span><span>$${Number(pricing?.total||0).toFixed(2)} CAD</span></div>`
  ].join('');
  const html = `<!DOCTYPE html><html><body style=\"font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial; background:#f6f9fc;\">\n  <div style=\"max-width:640px;margin:0 auto;padding:24px;\">\n    <div style=\"background:#fff;border-radius:10px;padding:28px;border:1px solid #e6ecf1;\">\n      <h1 style=\"margin:0 0 8px 0;color:#111;\">Hi ${simpleEscape(first_name||'there')},</h1>\n      <p>Your translation quote is ready.</p>\n      <div style=\"background:#f8f9fa;border-radius:6px;padding:16px;margin:16px 0;\">${rows}</div>\n      <div style=\"text-align:center;margin:22px 0;\">\n        <a href=\"${SITE_URL}/dashboard/quotes\" style=\"display:inline-block;background:#2563eb;color:#fff;text-decoration:none;padding:12px 20px;border-radius:6px;font-weight:600;\">View Quote</a>\n      </div>\n      <p style=\"color:#6b7280;font-size:13px;\">© ${new Date().getFullYear()} ${simpleEscape(FROM_NAME)}</p>\n    </div>\n  </div>\n</body></html>`;
  const sendSmtpEmail = new brevo.SendSmtpEmail();
  sendSmtpEmail.subject = subject; sendSmtpEmail.to = [{ email, name: first_name||'Customer' }]; sendSmtpEmail.sender = { email: FROM_EMAIL, name: FROM_NAME }; sendSmtpEmail.htmlContent = html; sendSmtpEmail.tags = ['hitl-quote-ready'];
  const resp = await apiInstance.sendTransacEmail(sendSmtpEmail); const messageId = resp?.messageId || resp?.response?.body?.messageId || null; await logEmailSent({ email_type:'hitl_quote_ready', recipient: email, provider_id: messageId, status:'sent' }); return { success:true, messageId };
}

export async function sendHitlRequestInfoEmail({ email, first_name, quote_number, message }) {
  if (!email) return { success: false, error: new Error('Missing recipient email') };
  const subject = `More Information Needed - Quote #${simpleEscape(quote_number||'')}`;
  const html = `<!DOCTYPE html><html><body style=\"font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial; background:#f6f9fc;\">\n  <div style=\"max-width:640px;margin:0 auto;padding:24px;\">\n    <div style=\"background:#fff;border-radius:10px;padding:28px;border:1px solid #e6ecf1;\">\n      <h1 style=\"margin:0 0 8px 0;color:#111;\">Hi ${simpleEscape(first_name||'there')},</h1>\n      <p>We need a bit more information to complete your quote:</p>\n      <div style=\"background:#f8f9fa;border-radius:6px;padding:16px;\">${simpleEscape(message||'')}</div>\n      <p style=\"margin-top:12px\">Reply to this email with your response.</p>\n      <p style=\"color:#6b7280;font-size:13px;\">© ${new Date().getFullYear()} ${simpleEscape(FROM_NAME)}</p>\n    </div>\n  </div>\n</body></html>`;
  const sendSmtpEmail = new brevo.SendSmtpEmail();
  sendSmtpEmail.subject = subject; sendSmtpEmail.to = [{ email, name: first_name||'Customer' }]; sendSmtpEmail.sender = { email: FROM_EMAIL, name: FROM_NAME }; sendSmtpEmail.htmlContent = html; sendSmtpEmail.tags = ['hitl-request-info'];
...
export async function sendHitlRejectedEmail({ email, first_name, quote_number, reason, message }) {
  ...
  const html = `<!DOCTYPE html><html><body style=\"font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial; background:#f6f9fc;\">\n  <div style=\"max-width:640px;margin:0 auto;padding:24px;\">\n    <div style=\"background:#fff;border-radius:10px;padding:28px;border:1px solid #e6ecf1;\">\n      <h1 style=\"margin:0 0 8px 0;color:#111;\">Update on Your Quote</h1>\n      <p>We cannot provide a quote at this time.</p>\n      <p><strong>Reason:</strong> ${simpleEscape(reason||'')}</p>\n      ${message ? `<div style=\\\"margin-top:8px\\\">${simpleEscape(message)}</div>` : ''}\n      <p style=\"color:#6b7280;font-size:13px;\">© ${new Date().getFullYear()} ${simpleEscape(FROM_NAME)}</p>\n    </div>\n  </div>\n</body></html>`;
  const sendSmtpEmail = new brevo.SendSmtpEmail();
  sendSmtpEmail.subject = subject; sendSmtpEmail.to = [{ email, name: first_name||'Customer' }]; sendSmtpEmail.sender = { email: FROM_EMAIL, name: FROM_NAME }; sendSmtpEmail.htmlContent = html; sendSmtpEmail.tags = ['hitl-rejected'];
  const resp = await apiInstance.sendTransacEmail(sendSmtpEmail); const messageId = resp?.messageId || resp?.response?.body?.messageId || null; await logEmailSent({ email_type:'hitl_rejected', recipient: email, provider_id: messageId, status:'sent' }); return { success:true, messageId };
}
